...
1. Build the payload with **`make_alert_payload`** (charting-tools MCP): `strategy_id`,
   `symbol`, `action` (buy/sell/close), `order_type` (market/limit), `account_track`
   (cfd/spreadbet), `price`, `atr`, `stop_atr_mult`, `risk_frac`, and a **unique
   `client_order_id`** (the idempotency key). The tool returns the payload and a validation
   result. (Required by the contract: `schema, strategy_id, client_order_id, account_track,
   symbol, action, order_type`; entries also need `atr`, `limit` orders need `limit_price`.)
...
4. To test end-to-end, dry-run the payload through the bridge pipeline with the `route-test`
   skill (no server needed), or **`sign_payload`** a sample with a test secret and POST it to
   a running bridge with `tools/post_alert.py`; the bridge must accept it and reject a
   tampered copy (401).
