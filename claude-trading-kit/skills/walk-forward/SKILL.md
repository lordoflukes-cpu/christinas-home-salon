...
3. **Interpret:**
   ...
   - WFE < 0.5, negative OOS, or few positive windows → the edge degrades out-of-sample;
     do **not** go live.
   - Read WFE **alongside** mean OOS Sharpe, never alone: if IS Sharpe is negative the ratio
     flips sign and looks fine — judge on a positive mean OOS Sharpe plus a healthy
     `oos_positive_windows` count first.
