...
4. Confirm the **track** is intended (tax: CFD = CGT above the allowance, spread-bet tax-free —
   see `tax-uk`) and the right venue is configured for it.
5. Only after preview + risk-check is clean should you place (or let the bot place) the order —
   and only with the kill-switch reachable (`/halt`).
