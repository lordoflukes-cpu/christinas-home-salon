...
1. **Recompute the risk independently** (use the backtest-engine `position_size` tool).
   Position size must risk ≤ the stated per-trade fraction (default 0.5%) at a
   `stop_atr_mult × ATR` stop. If the math doesn't tie out, reject.
